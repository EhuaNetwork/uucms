    ...
    $.ajax({
        ...
        success:function(res){
            if (1 == res.code) {
                ...
                if (res.data.is_wechat_followed && res.data.is_wechat_followed == -1) {
                    // layer.alert(res.msg, {icon: 5, title: false, closeBtn: 0}, function(index){
                    //     layer.close(index);
                        is_followed_wechat(res.data.users_id, false);
                    // });
                } else {
                    layer.closeAll();
...
function BestAnswer(obj, answer_id, users_id) {
    ...
    $.ajax({
        ...
        type: 'post',
        dataType: 'json',
        data: {answer_id:answer_id,users_id:users_id,askurl:askurl},
        success: function(res){
            layer.closeAll();
            if (1 == res.code) {
                layer.msg(res.msg, {time: 1000},function(){
...
function AnswerNew(obj) {
    ...
    url += 'new_answer=' + sort_order + '#comment';
    window.location.href = url;
}
...
// 收藏问题 undefined
function collection_details(obj)
{
    var users_id = ey_getCookie('users_id');
    if (0 >= users_id || undefined == users_id) {
        layerLogin();
        return false;
    }
    var url = $(obj).attr('data-url');
...
function is_followed_wechat(users_id, is_loading, closeBtn)
{
